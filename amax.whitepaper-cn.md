...
作为多链森林体系的创始母链，担当了所有其它子链的信任基石和底层价值通证来源。也就是说所有子链的原生代币必须来自于母链，用于子链的基础运作所需（例如交易燃料费使用）。因此确保母链的安全性和稳定性尤为重要。而区块链的安全性主要体现在去中心化程度和抗审查能力，这在技术的实现上需要做到以下几点：

1. 有足够多的节点运行区块链的数据，这样部分甚至全部的核心记账节点如果受到攻击，全网仍然可以及时恢复运行；
2. 如果其中有运行记账和验证的节点作恶，甚至是多个联合作恶，可以被全网节点发现并相应及时惩罚这些作恶节点；
3. 在就职的记账节点不工作的时候，可以被惩罚并且被有效替代出去；

Armonia核心团队针对以上建设目标，对母链采用了一种特全新的区块链共识机制：APOS(Armonia PoS) 是由DPoS和PoS混合而成的一种共识机制，而其它子链可以采用独立的共识机制运行来维护自己的网络数据。

APOS共识机制特征如下：
1. 全网有21个超级节点采用DPoS共识机制，经过开放式、不间断地投票选举出来，负责记账和验证主要区块；
2. 要参与超级节点候选，必须确保`$AMAX`的质押和投票总量大于100万枚，并且运行节点软件和同步好数据，随时可以参与记账和出块；
3. 所有未当选的或者未参与21个超级节点竞选的节点，都可以自我运行节点程序并参与POS的挖矿机制；

另外考虑到第一年Armonia母链主要是核心团队在维护和开发，并不会对外开放投票，所以在第一年内即使维护和运行了21个超级节点，每一个挖出的块并没有任何增发奖励。在第二年随着节点投票的开始，新的节点引入之后，每挖出的块都会有增发奖励给到该记账节点。
