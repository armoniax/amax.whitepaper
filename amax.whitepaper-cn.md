...
在多链分层架构下，Armonia会提供各个层的SDK，允许生态区块链建方可以选择模块、配置好参数，最后快速搭建一个个性化的单链，并且可以和生态内的各个子链和母链之间可自由交互。
...
这种方式下面，用户端可以发起需要触达到某一条链，或者一组多链，或者是所有链的交易，而每个多链体系下的节点通过L0-base的实现来相应提交到更上层处理，并且决定是否转发到整个网络里面去。但是如果交易可以提升到节点软件的上层模块，由于缺乏交易所需手续费，交易将无法执行而最终无法上链。
